...
class SmallDial extends StatelessWidget {
  final String title;
  final String unit;
  final double value;
  // 便利构造函数
  SmallDial(this.title, this.unit, this.value);
  // 表格间隔
  double tableSpace;
...
class DialPainter extends CustomPainter {
  ...
  void drawIndicatorPicture(Canvas canvas, Size size, double value) {
    ...
    // 计算角度
    if (value <= 10) {
      angle = -(10 - value) * unitAngle;
    } else {
      angle = (value - 10) * unitAngle;
    }
    canvas.save();
